...
export default function QrScannerTestPage() {
  ...
  // localStorage dan scan natijalarini o'qish (polling)
  useEffect(() => {
    const checkForScans = () => {
      const scansJson = localStorage.getItem("qr-scans");
      if (scansJson) {
        try {
    ...
    // Har 500ms da tekshirish
    const interval = setInterval(checkForScans, 500);
    checkForScans(); // Dastlab ham tekshirish
