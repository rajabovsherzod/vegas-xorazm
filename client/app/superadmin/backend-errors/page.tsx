/**
 * Backend Errors Dashboard
 * * Faqat backend xatoliklari
 */
...
import { AlertCircle, AlertTriangle, Info, Trash2, RefreshCw, Loader2, Server } from 'lucide-react';
...
export default function BackendErrorsPage() {
  ...
  // Get backend error logs from backend
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['backend-errors', filter, page],
    queryFn: () => logsService.getAll({ source: 'backend', level: filter, page, limit: 50 }),
    refetchInterval: 5000, // Har 5 soniyada yangilash
  });

  // Get stats (faqat backend uchun)
  const { data: statsData } = useQuery({
    queryKey: ['backend-errors-stats', filter],
    queryFn: async () => {
      const res = await logsService.getAll({ source: 'backend', level: filter, limit: 1000 });
      const logs = res.logs;
      return {
        total: logs.length,
        errors: logs.filter((e: ErrorLog) => e.level === 'error').length,
        warnings: logs.filter((e: ErrorLog) => e.level === 'warning').length,
        info: logs.filter((e: ErrorLog) => e.level === 'info').length,
      };
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['backend-errors'] });
      queryClient.invalidateQueries({ queryKey: ['backend-errors-stats'] });
    },
    onError: () => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                Backend xatoliklari monitoring dashboard
              </p>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-[#1C2C30] rounded-xl p-12 text-center shadow-sm border">
            ...
            <p className="text-muted-foreground">
              {filter === 'all' ? 'Hech qanday backend xatolik yo\'q' : `${filter} xatoliklari yo'q`}
            </p>
          </div>
        ) : (
          <>
            {errors.map((error: ErrorLog) => {
              // [Backend] prefiksini olib tashlash
              const cleanMessage = error.message.replace(/^\[Backend\]\s*/, '');
              ...
              return (
                ...
                >
                  ...
                  {error.context && (
                    <details className="mt-4">
                      <summary className="cursor-pointer text-sm font-medium text-[#00B8D9] hover:underline">
                        Context (Method, Status Code, Body, Query, Params)
                      </summary>
                      <pre className="mt-2 text-xs bg-gray-100 dark:bg-gray-900 p-4 rounded-lg overflow-x-auto">
